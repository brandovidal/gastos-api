...
- Shared expenses (P17): `shared-expense.parser.ts` reads "cena 120 con dany, mitad" / "a medias con dany" / "entre 3 con dany y juan" without the AI (names must be whole catalog names or aliases); the AI only gets "cena 120" and the draft keeps `sharedWith` (`{ personIds, parts }`). On save the expense keeps the user's part and one `owed_to_me` debt per person (and per installment) is created in the same transaction (`sharedDebts`). Subscriptions too (D72: a platform shared with the sister is charged to her every month as a debt).
