...
// Lists of P20 in Redis (D87): upcoming reminders, the latest notifications and the unread counter of the bell.
...
export class NotificationCache {
  ...
  async getUpcoming(fromMs: number, toMs: number): Promise<CalendarEvent[] | null> {
    return this.safely('getUpcoming', async (redis) => {
      if (!(await redis.exists(REDIS_KEYS.upcoming))) return null
      const members = await redis.zrangebyscore(REDIS_KEYS.upcoming, fromMs, toMs)
      return members
  ...
  async setUpcoming(events: CalendarEvent[]): Promise<void> {
    await this.safely('setUpcoming', async (redis) => {
      const multi = redis.multi().del(REDIS_KEYS.upcoming)
      // An empty calendar still has the key (a sentinel far in the past), so it is not rebuilt on every read
      multi.zadd(REDIS_KEYS.upcoming, 0, JSON.stringify({ sentinel: true }))
      // The index keeps two equal events (same day, name and amount) as two members of the set
      events.forEach((event, index) =>
        multi.zadd(REDIS_KEYS.upcoming, Date.parse(`${event.date}T00:00:00Z`), JSON.stringify({ ...event, index })),
      )
      await multi.exec()
    })
  }
  ...
  async getRecent(limit: number): Promise<Notification[] | null> {
    return this.safely('getRecent', async (redis) => {
      if (!(await redis.exists(REDIS_KEYS.recent))) return null
      const items = await redis.lrange(REDIS_KEYS.recent, 0, limit - 1)
  ...
  async setRecent(notifications: Notification[], unread: number): Promise<void> {
    await this.safely('setRecent', async (redis) => {
      const multi = redis.multi().del(REDIS_KEYS.recent)
      if (notifications.length) multi.rpush(REDIS_KEYS.recent, ...notifications.map((item) => JSON.stringify(item)))
      await multi.set(REDIS_KEYS.unread, unread).exec()
    })
  }
  ...
  async pushRecent(notification: Notification): Promise<void> {
    await this.safely('pushRecent', async (redis) => {
      if (!(await redis.exists(REDIS_KEYS.recent))) return
      const multi = redis
        .multi()
        .lpush(REDIS_KEYS.recent, JSON.stringify(notification))
        .ltrim(REDIS_KEYS.recent, 0, RECENT_NOTIFICATIONS - 1)
      if (!notification.readAt) multi.incr(REDIS_KEYS.unread)
      await multi.exec()
    })
  }
  ...
  async getUnread(): Promise<number | null> {
    return this.safely('getUnread', async (redis) => {
      const value = await redis.get(REDIS_KEYS.unread)
  ...
  async invalidateRecent(): Promise<void> {
    await this.safely('invalidateRecent', (redis) => redis.del(REDIS_KEYS.recent, REDIS_KEYS.unread))
  }
