...
const SETTINGS_ID = 'default' // one row until P23 adds userId
...
// Configuración ▸ Presupuesto: what adds to the budget besides fixed costs, cards and day to day
@Injectable()
export class BudgetSettingDBRepository {
  constructor(private readonly prisma: PrismaService) {}

  async get(): Promise<BudgetSettingsDbDto> {
    const row = await this.prisma.budgetSetting.findUnique({ where: { id: SETTINGS_ID } })
  ...
  async update(changes: BudgetSettingsUpdateDbDto): Promise<BudgetSettingsDbDto> {
    const row = await this.prisma.budgetSetting.upsert({
      ...
      create: { id: SETTINGS_ID, ...DEFAULT_BUDGET_SETTINGS, ...changes },
      update: changes,
    })
