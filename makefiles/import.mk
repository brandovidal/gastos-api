...
import-notion: env-file ## Notion CSV exports → Kogane ([DIR=<folder>], default Seguimiento financiero): report only; CONFIRM=yes saves; RESET=yes CONFIRM=yes deletes what came from Notion first [EMAIL=<user>, default the owner] [ENV=prod]
	USER_EMAIL="$(EMAIL)" $(DOTENV) tsx scripts/import-notion.ts "$(or $(DIR),$(NOTION_DIR))" $(if $(filter yes,$(CONFIRM)),--confirm) $(if $(filter yes,$(RESET)),--reset)
...
import-notion-reset: env-file ## Deletes only what came from Notion (rows with importKey, their payments and imp_batches/imp_rows); CONFIRM=yes [ENV=prod]
	...
	USER_EMAIL="$(EMAIL)" $(DOTENV) tsx scripts/import-notion.ts "$(or $(DIR),$(NOTION_DIR))" --reset-only
...
import-commitments: env-file ## Notion "Pago de Prestamos" and "Pago de Terreno" → loans and investments (P27) ([DIR=<folder>], default docs/migrations/notion): report only; CONFIRM=yes saves [ENV=prod]
	USER_EMAIL="$(EMAIL)" $(DOTENV) tsx scripts/import-commitments.ts "$(or $(DIR),$(NOTION_ROOT))" $(if $(filter yes,$(CONFIRM)),--confirm)
